// from https://github.com/ahrens/zfsondisk/commit/30ae2531352ab0d9d5ee9af82ce39a2cd45ccd79
// Luke Huang <lukehuang.ca@gmail.com>
...
static char *read_block(zdump_opts_t *opts, int compressed)
{
    ...
    if (pread(fd, buf, opts->psize, opts->offset) == -1) {
...
static json_object *zdump_fzap_leaf(zdump_opts_t *opts, json_object *json_parent)
{
    ...
    zap_leaf_chunk = (typeof(zap_leaf_chunk)) \
        (((uint8_t *)zap_leaf) + sizeof(zap_leaf->l_hdr)
         + /*sizeof(zap_leaf->l_hash)*/ 16 * (opts->lsize/32));
   // zap_leaf_chunk = (typeof(zap_leaf_chunk)) ZAP_LEAF_HASH_NUMENTRIES(opts->lsize);
